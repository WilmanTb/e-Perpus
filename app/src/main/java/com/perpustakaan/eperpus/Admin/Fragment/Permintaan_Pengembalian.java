...
public class Permintaan_Pengembalian extends Fragment {

    ViewGroup root;
    RecyclerView rc_listPermintaanPEngembalian;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        root = (ViewGroup) inflater.inflate(R.layout.fragment_permintaan_pengembalian, container, false);

        Daftar_Tunggu.x = 4;

        rc_listPermintaanPEngembalian = root.findViewById(R.id.rc_listPermintaanPengembalian);
        rc_listPermintaanPEngembalian.setHasFixedSize(true);
        rc_listPermintaanPEngembalian.setLayoutManager(new LinearLayoutManager(this.getActivity()));
        listTunggu = new ArrayList<>();
        adapterTunggu = new Tunggu_Adapter_Admin(getActivity(), listTunggu);
        rc_listPermintaanPEngembalian.setAdapter(adapterTunggu);
    ...
    private void getData() {
        dbPeminjaman.child("Peminjaman").orderByChild("status").equalTo("Menunggu konfirmasi pengembalian").addValueEventListener(new ValueEventListener() {
            @Override
    ...
    private void getKeranjang(){
        dbPeminjaman.child("KeranjangPinjam").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    for (DataSnapshot dataSnapshot : snapshot.getChildren()) {
                        String Key = dataSnapshot.getKey();
                        dbPeminjaman.child("KeranjangPinjam").child(Key).orderByChild("status").equalTo("Menunggu konfirmasi pengembalian").addValueEventListener(new ValueEventListener() {
                            @Override
                            public void onDataChange(@NonNull DataSnapshot snapshot) {
                                x = 0;
